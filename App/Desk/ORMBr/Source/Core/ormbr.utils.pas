...
type
  ...
  IUtilSingleton = interface
    ...
    function Delimiter(var AFields: TStringList; ADelimiter: Char; ATexto: string): Integer;
    function DateTimeToIso8601(AValue: TDateTime): string;
    function Iso8601ToDateTime(const AValue: string): TDateTime;
  end;

  TUtilSingleton = class(TInterfacedObject, IUtilSingleton)
  private
  class var
...
function TUtilSingleton.Delimiter(var AFields: TStringList; ADelimiter: Char;
  ATexto: string): Integer;
begin
  AFields.StrictDelimiter := True;
  AFields.Delimiter := ADelimiter;
  AFields.DelimitedText := ATexto;
...
begin
  ...
  case Length(AValue) of
    9:  if (AValue[1] = 'T') and (AValue[4] = ':') and (AValue[7] = ':') then
        begin
    ...
    10: if (AValue[5] = AValue[8]) and (Ord(AValue[8]) in [Ord('-'), Ord('/')]) then
        begin
