...
type
  ...
  public
    ...
    class function ObjectToJsonString(AObject: TObject; AOptions: TORMBrJsonOptions = [joDateIsUTC, joDateFormatISO8601]): string;
    class function JsonToObject<T: class, constructor>(const AJson: string; AOptions: TORMBrJsonOptions = [joDateIsUTC, joDateFormatISO8601]): T; overload;
    class function JsonToObject<T: class>(AObject: T; const AJson: string): Boolean; overload;
    class function JsonToObjectList<T: class, constructor>(const AJson: string): TList<T>;
  end;
...
begin
  Result := FJSONObject.ObjectToJSON(AObject, False);
end;

class function TORMBrJson.JsonToObjectList<T>(const AJson: string): TList<T>;
begin
